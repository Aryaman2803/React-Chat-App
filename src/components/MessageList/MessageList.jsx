        ...
        groupMessages(selectedChat.messages).map((m, index) => (
          <div key={index} className="chat-message">
            ...
            <div className="message-content">
              {m.map((individialMsg, index) => (
                <div key={index}>
                  <div className="message-text">{individialMsg.text}</div>
                  {!!individialMsg.attachments.length && (
                    <img
                      className="message-image"
                      src={individialMsg.attachments[0].file}
                      alt={individialMsg.id + "-attachment"}
                    />
                  )}
                </div>
              ))}
